...
router.get('/', async (req, res) => {
  const result = await pool.query(`
    ...
    LEFT JOIN pessoa pe ON pe.id = pes.pessoa_id`
  );
...
router.post('/', async (req, res) => {
  const { nome, descricao, data_inicio, data_fim,
  ...
  const result = await pool.query(`
    ...
    VALUES ($1,$2,$3,$4,$5,$6)
    RETURNING *`,
    [nome, descricao, data_inicio || null, data_fim || null,
      status || 'ATIVO', pesquisador_responsavel_id || null]
  );
  res.json(result.rows[0]);
});
...
router.put('/:id', async (req, res) => {
  ...
  const result = await pool.query(`
    UPDATE projeto SET
      ...
      status = $5, pesquisador_responsavel_id = $6
    WHERE id = $7
    RETURNING *`,
    [nome, descricao, data_inicio || null, data_fim || null,
      status, pesquisador_responsavel_id || null, req.params.id]
  );
  res.json(result.rows[0]);
});
